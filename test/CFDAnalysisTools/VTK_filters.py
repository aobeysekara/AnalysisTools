...
class VTKfilter():
    ...
    def CP(self,X1,Y1,Z1,D,res,p0,rho,U):
        parallel=False
        if self.vtuname.endswith(".pvtu"):
            parallel=True
        ############################ Plot from Numerical results
        ...
        delta_theta = np.longfloat(180)/res
        ...
        for i in range(res + 1):
            ...
            x = X1 + D/2 * np.longfloat(cos(radians(180 - delta_theta * i)))
            y = Y1 + D/2 * np.longfloat(sin(radians(180 - delta_theta * i)))
            z = Z1
        ...
        for j in range(points.GetNumberOfPoints()):
            ID=valid_ids.GetTuple(j)
            if ID[0] > 0:
                Dline.append(theta[j])
                p = data.GetPointData().GetScalars(data_name_field).GetComponent(j, 0)
                FS.append((float(p) - p0)/(0.5 * rho * U**2))

        return np.array(Dline), np.array(FS)
...
class Plotter2():
    ...
    def VTKplot(self, Xi,Yi,Xd,Yd,XN,FN):
        ...
        for i in range(len(self.X)):
            #if (float(self.Y[i][0]) != 0):
            xN.append(float(self.X[i]))#+0.5)#In this test case the origin is in -0.5
            yN.append(float(self.Y[i]))
        print(self.label)
        plt.plot(xN, yN, label=self.label,color=self.color, linestyle='dashed', linewidth=0.5, markersize=1)
